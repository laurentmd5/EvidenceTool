...
### G. Incidents Dépendances Microservices, Télémétrie & Latence SLA (`dependency`, `otel`)
...
| **Taux d'Erreurs HTTP Élevé (Mode A)** | Taux d'erreurs 5xx amont dépasse le seuil configuré. | Probe `otel.http_error_rate_high: FAIL` via requête Prometheus sécurisée. | 🛑 **BLOCK** (`HIGH_ERROR_RATE`) |
| **Pic de Latence P99 Détecté par APM** | Latence p99 dépasse le budget contractuel. | Probe `otel.p99_latency_high: FAIL` via requête Prometheus. | 🛑 **BLOCK** (`HIGH_LATENCY_P99`) |
| **Spans d'Erreurs dans Traces Tempo/Jaeger** | Traces distribuées actives avec spans d'erreurs. | Probe `otel.active_traces_failing: FAIL` via API Tempo/Jaeger. | 🛑 **BLOCK** (`ACTIVE_TRACES_ERRORING`) |

---

### H. Incidents Distribués Multi-Signaux & Raisonnement Causal Hybride (`distributed`, `telemetry`)
