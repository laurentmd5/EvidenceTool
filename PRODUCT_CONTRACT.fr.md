...
10. **Sémantique de Chemin Causal Continu & Graphe Ramifié (C15 / H2)** :
    ...
    - Dans un graphe ramifié ($A \longrightarrow B \longrightarrow S$ et $A \longrightarrow C \longrightarrow S$), les concaténations de nœuds formant des transitions inexistantes ($[A, B, C, S]$ où $B \longrightarrow C$ n'existe pas) sont interdites. Pour identifier le chemin vers les symptômes cibles, le BFS parcourt les voisins ordonnés par priorité d'arête (assurant un nombre minimal de sauts) ; `_build_causal_chain` sélectionne ensuite de manière déterministe parmi les chemins candidats en évaluant la profondeur du chemin, la somme des priorités d'arête et un départage lexicographique strict.
...
12. **Validation Strictement Fail-Closed & Zéro Coercition Silencieuse des Catalogues (C17, C18, C20-C23 / H4, H4.1)** :
    - Champs obligatoires (H4 / C17, C18) : Toute règle causale doit obligatoirement comporter des champs non vides pour `id`, `source`, `target` et `relation`. Toute omission lève une `ValueError` au chargement.
    - Analyse stricte des relations (H4 / C17) : Tout type de relation non reconnu (ex: faute de frappe `PROPAGATSE_TO`) DOIT échouer immédiatement en levant une `ValueError`. La conversion silencieuse vers `PROPAGATES_TO` est formellement interdite.
    - Zéro coercition silencieuse pour les priorités (H4.1 / C20) : `priority` doit être strictement un entier. Les chaînes (ex: `"HIGH"`), booléens ou flottants DOIVENT lever une `ValueError`. Le repli silencieux vers `priority = 0` est interdit.
    - Validation stricte des conditions (H4.1 / C21) : `conditions` doit être strictement un dictionnaire ou une liste de conditions avec des statuts `EvidenceStatus` valides (`PASS`, `FAIL`, `UNKNOWN`). Tout format invalide, clé vide ou délimiteur absent DOIT lever une `ValueError`.
    - Indicateurs booléens stricts (H4.1 / C22) : `is_root_cause_candidate` et `is_surface_symptom` doivent être des booléens natifs stricts. Les chaînes ("maybe", "true") ou entiers lèvent une `ValueError`.
    - Structure stricte des règles (H4.1 / C23) : Tout élément de `causal_rules` doit obligatoirement être un dictionnaire. Les éléments non dictionnaires lèvent une `ValueError` au lieu d'être ignorés silencieusement.
    - Principe fondamental : *Un catalogue causal ne doit jamais être interprété différemment de ce que son auteur a déclaré.* Corrompu ou ambigu, le chargement échoue immédiatement.
